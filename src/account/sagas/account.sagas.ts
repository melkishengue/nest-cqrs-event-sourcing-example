    ...
    return events$
      .pipe(
        ...
        map(event => {
          ...
          return new CreditAccountCommand(event.receiverAccountId, event.accountId, event.amount);
        }),
      );
  }
  ...
  receiverAccountCreditedFailed = (events$: Observable<any>): Observable<ICommand> => {
    ...
    .pipe(
      ...
      map(event => {
        ...
        return new CreditAccountCommand(event.accountId, event.receiverAccountId, event.amount);
      }),
    );
  }
}
